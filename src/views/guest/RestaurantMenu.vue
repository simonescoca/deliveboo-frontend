<template>
    <div class="loader-cont position-relative" v-if="loading">
        <span class="loader position-absolute"></span>
    </div>
    <!-- ---Sezione dati ristorante--- -->
    <header class="position-relative" v-if="!loading">
    ...
    <div v-if="!loading" class="d-flex justify-content-center align-items-center flex-wrap container mb-5">
    ...
    <main class="position-relative" v-if="!loading">
        ...
        <div class="container pb-5">
            <section>
                ...
                <div class="d-flex flex-wrap justify-content-around align-items-center"
                    v-if="!this.nessunaCategoriaPresentePrimi">
                    <div class="myCard col-3-custom mx-auto mb-5" v-for="dish in firstCourse"
                        :class="!showSelectedDish(dish.categories) ? 'notSelected' : ''">
                        <div class="content-info">
                            ...
                            <p class="btn btn-yellow customCursor" :disabled="activeButton == false"
                                @click="addToCart(dish)">Aggiungi al Carrello</p>
                        </div>
                        <div class="content" :class="showDescription[dish.id] ? 'description-hidden' : ''">
                            ...
                            <div class="description">
                                <p class="title fw-bold">
                                    {{ dish.name }}
                                    <i v-if="dish.available === 1" class="fa-solid fa-circle-info ms-2"
                                        @click="seeDescription(dish)"></i>
                                    <i v-else class="fa-solid fa-ban ms-2"></i>
                                </p>
                                <p class="price">
                                    {{ dish.price.toFixed(2) }}
                                </p>
                                <button v-if="dish.available === 1" class="myCart-button"
                                    @click="dish.cartClicked = true, addToCart(dish)"
                                    :class="dish.cartClicked === true && activeButton === true ? 'myClicked' : ''">
                ...
                <div class="d-flex flex-wrap justify-content-around align-items-center"
                    v-if="!this.nessunaCategoriaPresenteSecondi">
                    <div class="myCard col-3-custom mx-auto mb-5" v-for="dish in secondCourse"
                        :class="!showSelectedDish(dish.categories) ? 'notSelected' : ''">
                        <div class="content-info">
                            <p class="title fw-bold">
                                {{ dish.name }}
                ...
                <div class="d-flex flex-wrap justify-content-around align-items-center"
                    v-if="!this.nessunaCategoriaPresenteDessert">
                    <div class="myCard col-3-custom mx-auto mb-5" v-for="dish in dessertCourse"
                        :class="!showSelectedDish(dish.categories) ? 'notSelected' : ''">
                        <div class="content-info">
                            <p class="title fw-bold">
                                {{ dish.name }}
...
.loader:after {
    ...
    filter: drop-shadow(0 0 1rem rgba(239, 116, 54, 0.75));
}

.loader:before {
    width: 100%;
    padding-bottom: 100%;
    box-shadow: inset 0 0 0 1rem rgba(233, 193, 173, 0.75);
...
.loader:after {
    ...
    box-shadow: 0 0 0 0 rgba(215, 148, 114, 0.75);
    background-color: $primarysoft;
...
@keyframes pulsIn {
    0% {
        box-shadow: inset 0 0 0 1rem rgba(239, 116, 54, 0.75);
        opacity: 1;
    }

    50%,
    100% {
        box-shadow: inset 0 0 0 0 rgba(239, 116, 54, 0.75);
        opacity: 0;
    }
}
...
@keyframes pulsOut {
    ...
    0%,
    50% {
        box-shadow: 0 0 0 0 rgba(239, 116, 54, 0.75);
        opacity: 0;
    }

    100% {
        box-shadow: 0 0 0 1rem rgba(239, 116, 54, 0.75);
        opacity: 1;
    }
}
